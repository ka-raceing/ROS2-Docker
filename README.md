...
`as` and re-owns `/home/as`; then it drops to `as`, sources ROS and the workspace, and execs the
command. Bind-mount only under `/workspaces` and `/data`, never inside `/home/as`.

`docker exec` bypasses the entrypoint: pass `-u as` (VS Code: `"remoteUser": "as"`). An
interactive shell gets the environment from `~/.bashrc`; a one-off command goes through the
entrypoint: `docker compose exec -u as car /usr/local/bin/entrypoint.sh ros2 topic list`.
...
commits are all pinned in `docker/Dockerfile` and `.gitmodules`. osqp, osqp-eigen, qpmad and Ipopt
